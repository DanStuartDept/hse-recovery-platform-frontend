...
**Goal:** Add `not-found.tsx`, `error.tsx`, and `global-error.tsx` to the hse-app-template so 404s and runtime errors show user-friendly pages matching the HSE design system.

**Architecture:** Three files in `apps/hse-app-template/src/app/`. `not-found.tsx` is a Server Component matching the live hse.ie 404 page. `error.tsx` and `global-error.tsx` are Client Components (React error boundaries) that log errors via `@repo/logger` in local dev. All use `Container`, `Row`, `Col` from the HSE design system.
...
|------|--------|----------------|
| `apps/hse-app-template/package.json` | Modify | Add `@repo/logger` dep, testing devDeps |
| `apps/hse-app-template/vitest.config.mts` | Modify | Switch to `createVitestConfig` with jsdom |
| `apps/hse-app-template/src/vitest.setup.ts` | Create | Import jest-dom matchers |
| `apps/hse-app-template/src/app/not-found.tsx` | Create | 404 page matching hse.ie |
| `apps/hse-app-template/src/app/not-found.test.tsx` | Create | Render test for 404 page |
| `apps/hse-app-template/src/app/error.tsx` | Create | Route-segment error boundary |
| `apps/hse-app-template/src/app/error.test.tsx` | Create | Render + behaviour test |
| `apps/hse-app-template/src/app/global-error.tsx` | Create | Root-layout error fallback |
| `apps/hse-app-template/src/app/global-error.test.tsx` | Create | Render + behaviour test |
...
**Files:**
- Modify: `apps/hse-app-template/package.json`
- Modify: `apps/hse-app-template/vitest.config.mts`
- Create: `apps/hse-app-template/src/vitest.setup.ts`

- [ ] **Step 1: Add @repo/logger as an app dependency**

```bash
cd apps/hse-app-template && pnpm add @repo/logger@"workspace:*"
```

- [ ] **Step 2: Add React testing libraries as devDependencies**

```bash
cd apps/hse-app-template && pnpm add -D @testing-library/react@^16.2.0 @testing-library/jest-dom@"catalog:" @repo/vitest-config@"workspace:*"
```
...
Replace the contents of `apps/hse-app-template/vitest.config.mts` with:
...
```bash
cd apps/hse-app-template && pnpm vitest run
```
...
```bash
git add apps/hse-app-template/package.json apps/hse-app-template/vitest.config.mts apps/hse-app-template/src/vitest.setup.ts pnpm-lock.yaml
...
**Files:**
- Create: `apps/hse-app-template/src/app/not-found.test.tsx`
- Create: `apps/hse-app-template/src/app/not-found.tsx`
...
```bash
cd apps/hse-app-template && pnpm vitest run src/app/not-found.test.tsx
```
		...
		<Container>
			...
			<Row>
				<Col width="two-thirds">
					...
					<p className="hse-body-reg-m">
						<a href="https://www2.hse.ie/contact/">Contact us</a> if you have a question or want to give
						feedback.
					</p>
				</Col>
			</Row>
...
```bash
git add apps/hse-app-template/src/app/not-found.tsx apps/hse-app-template/src/app/not-found.test.tsx
...
**Files:**
- Create: `apps/hse-app-template/src/app/error.test.tsx`
- Create: `apps/hse-app-template/src/app/error.tsx`
...
```bash
cd apps/hse-app-template && pnpm vitest run src/app/error.test.tsx
```
...
```bash
cd apps/hse-app-template && pnpm add -D @testing-library/user-event@^14.4.0
```

- [ ] **Step 6: Commit**

```bash
git add apps/hse-app-template/src/app/error.tsx apps/hse-app-template/src/app/error.test.tsx
...
**Files:**
- Create: `apps/hse-app-template/src/app/global-error.test.tsx`
- Create: `apps/hse-app-template/src/app/global-error.tsx`
...
```bash
cd apps/hse-app-template && pnpm vitest run src/app/global-error.test.tsx
```
...
```bash
git add apps/hse-app-template/src/app/global-error.tsx apps/hse-app-template/src/app/global-error.test.tsx
...
```bash
turbo run typecheck --filter=hse-app-template
```
...
```bash
turbo run test --filter=hse-app-template
```
...
```bash
turbo run lint --filter=hse-app-template
```
