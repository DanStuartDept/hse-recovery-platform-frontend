...
**Architecture:** Source-only package with two entry points — `@repo/app-config` (client-safe NEXT_PUBLIC_* config) and `@repo/app-config/server` (server-only secrets). Zod validates at import time; throws on missing required vars, returns `undefined` for missing optional vars. Test escape hatch provides a stub config when `NODE_ENV=test`.
...
**Files:**
- Create: `apps/hse-app-template/.env.example`

- [ ] **Step 1: Create `apps/hse-app-template/.env.example`**
...
In `apps/hse-app-template/package.json`, add to `dependencies`:
...
```bash
git add apps/hse-app-template/.env.example apps/hse-app-template/package.json pnpm-lock.yaml
git commit -m "feat(hse-app-template): add .env.example and app-config dependency"
```
