...
| `dictionaries/en.json`       | Shared English dictionary                                       |
...
| `src/dictionaries/en.json`       | Create — app English dictionary                        |
...
- Create: `packages/i18n/dictionaries/en.json`
...
describe("loadDictionary", () => {
  ...
  it("falls back to default locale for missing keys", async () => {
    const flat = await loadDictionary("ga", mockLoaders, "en");
    // ga app dict has "app.title" but not "common.hello" override
    expect(flat["app.title"]).toBe("Mo Aip");
    // "common.hello" should come from ga shared (not en fallback) since ga shared has it
...
export function createI18nProxy(config: I18nConfig) {
  ...
  return function proxy(request: NextRequest): NextResponse {
    ...
    // Default locale — rewrite internally (URL stays clean, no /en/ prefix)
    const url = request.nextUrl.clone();
...
- Create: `apps/hse-multisite-template/src/dictionaries/en.json`
...
Save to: `apps/hse-multisite-template/src/dictionaries/en.json`
...
import type enIe from "@/dictionaries/en.json";
...
# Visit http://localhost:3000/en/ → should redirect to /
