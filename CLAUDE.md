...
- **SEO metadata**: Layout defines branding defaults (title template, `metadataBase`, default description) as app-level constants. The catch-all route's `generateMetadata` calls `generatePageMetadata` from `@repo/wagtail-cms-mapping` for per-page title, description, and canonical URL from CMS fields (`seo_title`, `search_description`).
