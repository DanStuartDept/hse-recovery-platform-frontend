...
- **CMS content flow**: `CMSClient` (from `@repo/wagtail-api-client`) fetches data using `config.cms` from `@repo/app-config`; response shapes validated via Zod schemas in `@repo/wagtail-cms-types`; `createCMSRenderer` (from `@repo/wagtail-cms-mapping`) maps data to React components. `generatePageMetadata` (same package) translates CMS page fields to Next.js `Metadata` objects.
...
- **i18n**: `@repo/i18n` provides locale-aware routing (`createI18nProxy`), dictionary loading (`getDictionary`), and client-side translation (`DictionaryProvider` / `useDictionary`). Default locale (`en`) is hidden from URLs; non-default locales get a prefix (`/ga/`).
- **SEO metadata**: Layout defines branding defaults (title template, `metadataBase`) as app-level constants. The catch-all route's `generateMetadata` loads the locale dictionary for a translatable default description, then calls `generatePageMetadata` from `@repo/wagtail-cms-mapping` for per-page title, description, and canonical URL from CMS fields (`seo_title`, `search_description`).
- **Caching**: Three-layer strategy — `generateStaticParams` pre-renders all CMS pages at build time; ISR (`next: { revalidate: 3600 }` — 1 hour) is the safety net; on-demand `revalidatePath` via `/api/revalidate/` webhook handles real-time cache invalidation when content is published in Wagtail. `dynamicParams = true` allows new pages to be rendered on-demand. `next.config.ts` enables `logging.fetches.fullUrl` on localhost for debugging cache behaviour.
