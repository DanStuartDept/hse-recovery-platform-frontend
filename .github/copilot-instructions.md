...
- **SEO metadata**: Layout defines branding defaults (title template, `metadataBase`) as app-level constants. The catch-all route's `generateMetadata` loads the locale dictionary for a translatable default description, then calls `generatePageMetadata` for per-page title, description, and canonical URL from CMS fields.
